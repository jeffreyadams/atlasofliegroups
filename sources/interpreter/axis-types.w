...
{ auto P_kind = sub_tp.raw_kind(), Q_kind = pattern.raw_kind();
  ...
  if (P_kind!=Q_kind and P_kind!=tabled and P_kind!=variable_type)
    return false;
  switch(P_kind)
  {
  case tabled:
     return Q_kind==tabled ? sub_tp.type_nr()==pattern.type_nr()
            : unify(sub_tp.expansion(),pattern);
  case variable_type:
  ...
  case undetermined_type: assert(false);
  }
  return false; // keep compiler happy
}
