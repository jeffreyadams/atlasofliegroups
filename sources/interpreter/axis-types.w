...
bool unify_to(type_expr& pattern, unsigned int fix_count) const;
...
get the value that was substituted for it the first time), but not always what
is desired (we do not for instance want the type of a variable to change because
a specialised form of its value is used somewhere); therefore a |const| version
of this method is also available under the name |unify_to|. Any type variables
of |pattern| will be treated as primitive if less than our |floor()|, and
otherwise as universally quantified (not to be specialised); it is the caller's
