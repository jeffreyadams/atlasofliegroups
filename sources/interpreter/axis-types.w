...
{ switch(tag)
  { case undetermined_type: out << '*'; break;
    ...
    case variable_type: out << 'A' + typevar_variant; break;
...
type_ptr mk_row_type(type_ptr&& c)
{@; return std::make_unique<type_expr>(type_expr::row(std::move(*c))); }
...
provided to easily assign a type expression to a type variable, test
whether this has been done, and to renumber type variables taking into account
that those that have been assigned to will disappear from the type when
performing substitution.

@< Type definitions @>=
struct type_assignment
{ const unsigned int var_start; // first |variable_type| number that may vary
  std::vector<const_type_p> equiv; // variable substitutions go here
@)
  type_assignment(unsigned int fix_nr, unsigned int var_nr)
  : var_start(fix_nr)
  , equiv(var_nr,nullptr)
  {}
  const_type_p equivalent (unsigned int i) const
  {@; return i<var_start ? nullptr : equiv[i-var_start]; }
...
into a |type_expr| value. The latter produces a freshly built |type_expr|, owned
by the |type_ptr| it returns, and neither takes ownership of (parts of) their
argument types, which are therefore passed by (non owning) constant reference.
The final argument to |can_unify| serves a technical purpose: initially |false|,
setting it to |true| in certain recursive calls allows ensuring termination. The
|type_expr| arguments here should not contain any undetermined subexpressions;
to achieve this, a caller may need to replace each such occurrence by a fresh
(only used here) type variable, and add a slot for it in the |type_assignment|.

@< Declarations of exported functions @>=
bool can_unify(const type_expr& P, const type_expr& Q, type_assignment& assign,
               bool freeze=false);
type_ptr substitution(const type_expr& M, const type_assignment& assign);
...
the way back in a recursive traversal of the type using the |mk| family
type-building functions, all of which return a |type_ptr|. In most cases we just
call such a function matching the kind of type we are at, using recursion in its
arguments to transform the type components before it gets called.
...
type_ptr substitution(const type_expr& M, const type_assignment& assign)
{ switch (M.raw_kind())
  { case primitive_type: return mk_prim_type(M.prim());
    case function_type: return
      mk_function_type(std::move(*substitution(M.func()->arg_type,assign)),
                      std::move(*substitution(M.func()->result_type,assign)));
    case row_type: return mk_row_type(substitution(M.component_type(),assign));
    case tuple_type:
    case union_type:
    { dressed_type_list aux;
      for (wtl_const_iterator it(M.tuple()); not it.at_end(); ++it)
        aux.push_back(std::move(*substitution(*it,assign)));
      return (*(M.raw_kind()==tuple_type ? &mk_tuple_type : &mk_union_type))
             (aux.undress());
    }
    case tabled: return mk_tabled_type(M.type_nr());
    case variable_type:
      @< If a type is associated in |assign| to  type variable
         |M.typevar_count()|, return a copy of that into which substitution was
         recursively applied; otherwise return a copy of the type variable
         itself @>
...
{ auto c = M.typevar_count();
  ...
  return p==nullptr
    ? mk_type_variable(assign.renumber(c)) : substitution(*p,assign);
}
