...
|assign.equivalent| instead of they |definiens|. Unlike tabled types, that
cannot refer directly to another one, unification can make two polymorphic type
variables equal (through an assignment in one direction, to avoid circularity);
therefore we use a |while| loop to iterate replacement until something other
than a type variable is found. Type variables numbered below |assign.threshold|
do not have an |equiv| entry in |assign|, so they will fall through these loops,
just like polymorphic variables without assignment to them. In either case we
return |true| if $P=Q$, which is in fact the only way to succeed for type
variables below the threshold. For those above the threshold that are not
identical to the other one of $\{P,Q\}$, we call |assign.set_equivalent| to try
to assign the other type expression to it, and return whether that succeeds. The
way |set_equivalent| handles the case of a type expression that cannot be
assigned because it refers back to the same type variable, namely by returning
|false|, means that this just makes the unification fail; this is precisely what
