...
expressions (and one spare, so $4$ in all), one in which the value of
the \emph{first} expression is retained as final value and the second
expression is then evaluated without using its value (the \&{next} keyword),
and one where the result of the left operand determines whether the right
operand will be evaluated or not (the |do| in |while| loops). The final spare
one is a variant of the latter (indicating that the condition has been
negated) will never be generated by the parser, but car arise due to rewriting
of the syntax tree during expression analysis.

@< Enumeration tags for |expr_kind| @>=
seq_expr, next_expr, do_expr, neg_do_expr, @[@]
