...
/*!
  ...
  a |bitvector::BitMatrix|, i.e., the image of that matrix
*/
template<size_t dim> Subspace<dim>::
...
template<size_t dim>
  bool Subspace<dim>::contains(const bitvector::BitVector<dim>& v) const
    { return mod_image(v).isZero(); }

template<size_t dim>
  bool Subspace<dim>::contains(const bitvector::BitVectorList<dim>& m) const
    {
...
*/
template<size_t dim>
bitvector::BitVector<dim> Subspace<dim>::representative
  (const bitvector::BitVector<dim>& v) const
{
  assert(v.size()==rank());
  ...
  bitvector::BitVector<dim> w = combination(d_basis,rank(),pv.data());
...
void Subspace<dim>::apply(const bitvector::BitMatrix<dim>& r)
{
  ...
  bitvector::BitVectorList<dim> b; b.reserve(dimension());
     ...
     const bitvector::BitMatrix<dim>& m)
{
  ...
  bitvector::BitMatrix<dim> result(dest.dimension(),0);
...
template
  bitvector::BitMatrix<constants::RANK_MAX> subquotientMap
   (const Subquotient<constants::RANK_MAX>&,
    const Subquotient<constants::RANK_MAX>&,
    const bitvector::BitMatrix<constants::RANK_MAX>&);
