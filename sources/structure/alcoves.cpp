...
void extend_affine_component
  (sl_list<WeylElt>& orbit,
   ...
   RootNbrSet comp, RootNbrSet stab)
{
...
public:
  ...
  unsigned int index() const { return order; }
}; // |center_classifier|

center_classifier::center_classifier(const RootSystem& rs)
  : rs(rs), flags(1u<<rs.rank()), order(rs.type().Cartan_determinant())
{
...
std::vector<sl_list<WeylElt> > FPP_facet_ws
  (const RootSystem& rs, const WeylGroup& W, const RootNbrSet& stabilising_walls)
{
  ...
  { assert(not stabilising_walls.contains(comp));
    ...
    const RootNbr last = roots.back();
    roots.pop_back();
    const int_Matrix Cartan = rs.Cartan_matrix(roots);
    dynkin::Lie_type(Cartan); // will throw if it is not valid
    std::vector<WeylElt> reflections;
    for (Generator i=0; i<roots.size(); ++i)
      reflections.push_back(W.element(rs.reflection_word(roots[i])));
  ...
  (const RootDatum& rd, const WeylGroup& W, const RatWeight& gamma)
{
  ...
  RootNbrSet stabilising_walls(walls.capacity());
  ...
  auto coset_lists = FPP_facet_ws(rd,W,stabilising_walls);

  center_classifier cc(rd);
  int_Matrix result;
  { size_t prod=1;
    for (const auto& list : coset_lists)
      prod *= list.size();
    assert(prod%cc.index()==0);
    result = int_Matrix(rd.rank(),prod/cc.index()); // allocate all space here
  }
  size_t j=0;

  std::vector<sl_list<WeylElt>::weak_const_iterator> its;
  std::vector<WeylElt> ws; // partial products from right to left
  its.reserve(coset_lists.size());
  ws.reserve(coset_lists.size());
  for (const auto& list : coset_lists)
  {
    its.push_back(list.wcbegin());
    ws.push_back(WeylElt());
  }

  Weight numer (gamma.numerator().begin(),gamma.numerator().end());
  std::vector<int_Vector> shifts(1ul<<rd.semisimple_rank());
  for (unsigned long d=0; d<shifts.size(); ++d)
  { RankFlags descents(d);
    if (cc.is_for_FPP(descents))
    { shifts[d]=int_Vector(rd.rank(),0);
      auto shift_adj = cc.shift(descents);
      for (unsigned int i=0; i<shift_adj.size(); ++i)
	shifts[d] += rd.simpleRoot(i)*(gamma.denominator()*shift_adj[i]);
    }
  }
  while(true)
  {
    RankFlags descents; // for |ws[0]|
    for (weyl::Generator s=0; s<rd.semisimple_rank(); ++s)
      descents.set(s,W.has_descent(s,ws[0]));
    if (cc.is_for_FPP(descents))
    {
      assert(j<result.n_columns());
      auto image = W.image_by(rd,ws[0],numer);
      result.set_column(j++,image+shifts[descents.to_ulong()]);
    }
    unsigned i=0;
    while(i<its.size() and coset_lists[i].at_end(++its[i]))
    {
      its[i]=coset_lists[i].wcbegin();
      ++i;
    }
    if (i==its.size()) // then we reached the end of our traversal
      break;
    ws[i] = i+1==its.size() ? *its[i] : W.prod(*its[i],*its[i+1]);
    while (i-->0)
      ws[i]=ws[i+1];
  } // |while(true)|
