...
TitsGroup::TitsGroup(const rootdata::RootDatum& rd,
  ...
  , d_rank(rd.rank())
  , d_simpleRoot(rd.semisimpleRank())
  , d_simpleCoroot(rd.semisimpleRank())
  , d_involution(d.transposed())
{
  for (size_t j = 0; j < rd.semisimpleRank(); ++j) {
    lattice::mod2(d_simpleRoot[j],rd.simpleRoot(j));
    lattice::mod2(d_simpleCoroot[j],rd.simpleCoroot(j));
  }
}
...
/*!
  ...
  Weyl group!
*/
void TitsGroup::mult(TitsElt& a, weyl::Generator s) const
...

{
  ...
  reflection(a.t(),s);
// mult performs mult of w by s, returns -1 in case of length reduction
  if (d_weyl->mult(a.w(),s) == -1)
    a += d_simpleCoroot[s];
}
...
void TitsGroup::leftMult(TitsElt& a, weyl::Generator s) const
		      ...
		      const rootdata::RootDatum& rd)
{
  ...
  for (size_t j = 0; j < n; ++j) {
    ...
    result[j] = std::find(sr.begin(),sr.end(),v) - sr.begin();
  }
