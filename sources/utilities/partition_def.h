...
/*!
  ...
  The type F is that of some function object that can be given a pair of
  ...
  range [0,n[. It is assumed that for any fixed i, the function a(i,.) is a
  permutation of [0,n[; thus the action parameter a defines c generators of a
  permutation subgroup of Sym([0,n[). The orbits for this action are returned
  as a Partition object in pi.

  The algorithm is straightforward orbit generation, using a set B of elements
  that have not yet joined any orbit, and a collection S of elements that are
  in the current orbit but whose acted-upon images have yet to be generated.
  The set B is represented as a bitmap on [0,n[, initially full, while S is
  realized as a stack (but it could equally well have been a queue), initially
  empty.

  While B is not empty :

    - move an element (the first one left) from B to (the currently empty) S;
      start a new orbit with this element
