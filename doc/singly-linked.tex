...
\subsection{Simpler lists and weaker iterators}
...
$\textit{Container}$. The \textit{Container} class must provide the
mentioned methods. If \std{stack}$\ang T$ or \std{queue}$\ang
T$ are used without specifying a container type as second template argument, the
container type \std{deque}$\ang T$ will be used as default.

Considering whether it is possible to instead use these container adaptors with
the container class templates \slls. or \smpl., we see that it should be
possible to use $\std{queue}\langle T,\slls.\ang T\rangle$, since \slls. defines
methods \textit{push\_back} (and \textit{emplace\_back}), \textit{front},
\textit{pop\_front}, and \textit{empty} (but \smpl. lacks \textit{push\_back},
so it cannot be used here). For \std{stack} on the other hand, there is a
difficulty in that it wants to use \textit{back} and \textit{pop\_back} methods
of the container (a choice probably motivated by the idea of using \std{vector}
as container), which neither of our singly linked list containers define. But
...
(exclusively) as template argument to \std{stack}. It is derived from \smpl.,
and apart from its constructors, it only defines methods \textit{push\_back},
...
\textit{front}, and \textit{pop\_front} of its base class. (Being limited to
defining only the methods that \std{stack} uses and inheriting the rest, the
result is a rather strange container type, where for instance \textit{front} is
synonym for \textit{back}. The inheritance from \slls. is only relevant for
...
can it be efficiently implemented without recording additional information. We
consider it however to be an error on the part of the \Cpp\ specification to
require a \textit{back} method here, since access the most recently added item
is not part of the queue abstract data type. Indeed in none of the many places
where we use queues in our software do we have any use for such a method. We
have chosen to use $\std{queue}\langle T,\slls.\ang T\rangle$ in these places;
since methods of a class template that are never called are not compiled either,
this causes no problems. Similarly $\std{stack}\langle T,\mspl.\ang T\rangle$
cannot handle calls to \textit{size}, but code using a stack should not have any
need for such calls, and in the absence of such calls will compile.
...
will ever need. On the other hand there is some functionality that one can add
at no cost to that of \std{queue} and \std{stack} when using a simply linked
list as underlying container: a constructor that takes a \std{initializer\_list}
...
constructing a new node). Therefore, rather than overriding the default
container every time a queue or stack variable is introduced, we provide (in our
namespace) templates for classes \queue. and \stack. that are \emph{derived} from
...
The functionality we implemented for singly linked lists, as described above,
was developed in a fairly incremental manner, in the context of the project
Atlas of Lie groups and Representations; the commit where the class
templates for singly linked lists were first introduced was made on June~20,
2014. In the course of time the definitions were refined and extended with
methods, with the goal of covering the functionality of \frwl..

(although at some point, notably while
writing the current paper, an considerable effort was made to make it conform as
much as possible to the standard library conventions for container types, and
retain proximity to the \frwl. specification).

\subsection{The Atlas project}
Lets us say a few words about the Atlas project. In the context of this project,
a special purpose computer algebra package is being developed, originally
conceived and written by Fokko du Cloux. Its implementation has two major
components of rather different nature: a library of specialised algorithms on
one hand, and a user programming interface on the other. The library centres
around a number of discrete, graph-like structures (root systems, double cosets
$K\backslash G/B$, blocks of representations) that are generated dynamically,
and whose vertices with associated data are stored in a vector structure, the
mathematical values represented by such a vertex being henceforth most often
represented by just an index into that vector. The programming interface has a
structure common to many formal language processing tools such as compilers and
interpreters: input gathering functions, a lexical analyser, a (generated)
parser, a type checking system that also transforms expressions and functions
into executable tree structures, an evaluator for such structures, and some
minor output handling facilities.
...
(implicitly) use \std{deque}, or in some cases to use explicit definitions of
linked structures, ones containing a pointer to the same structure. (There could
also have been places where previously \std{list} was used, but in fact there
were not. As for \frwl., although it had become officially part of the language
when we introduced \slls., we would at that point in time have considered it,
from a portability point of view, too new to use anyway.)
...
implementation. The latter case includes all uses of stacks and queues, where we
used the class templates mentioned above that derive from \std{stack}
respectively \std{queue} instance with linked lists as underlying container
type. This was a drop-in replacement, with a small added convenience that we
added for placing one or more values into the data structure at construction.
The change leads to a substantially more modest memory footprint, especially in
cases like ours where the size of the stack or queue typically remains small (a
\std{deque} needs around $600$ bytes of storage even when it is empty, as
opposed to about $20$ bytes for an empty \slls. instance) and simpler
implementation of operations. In the Atlas library, these data types are used in
about a dozen places, generally to hold yet-to-process elements in some orbit
generation process or similar kind of closure operation. Either a stack or a
queue would do for this purpose, with a stack giving more of a depth-first
approach versus a queue leading to more breadth-first processing; we generally
prefer the latter, as we expect it to give a smoother evolution of the storage
needed.
