...
cases there are no important differences with respect to using \vect.
instead, as we would have done if we had not had \slls. available. The most
common reason to prefer \slls. is when successive items are produced by a
process for which the total number of items is not predictable. Even when
prediction is possible but complicated, the usage of \vect. poses the
question of whether to perform the calculation and \textit{reserve} the precise
amount of entries, or let the algorithm for growing the allocated size of a
vector by exponentially growing chunks do its work in amortised linear time,
with the inconvenience of possibly allocating substantially more space than
necessary. By contrast, using \slls., no decision to reserve anything is
necessary, and producing the list otherwise proceeds using repeated calls of
\textit{push\_\!back} just like it would when using a vector. In some cases we
want to store the result as a vector for reasons of efficiency of random access
or of compactness, but even then this can simply be achieved by conversion using
a final call of \textit{to\_\!vector}.
...
given Dynkin diagram, the transducer tables for our implementation of finite Coxeter groups (see~\ref{A transducer approach to Coxeter groups, J.
  Symb. Comput. 1999}), or the structured set $K\backslash{G/B}$ of $K$-orbits
on $G/B$ (affectionately called KGB-set within the Atlas project). In such
cases, the time spent constructing a table is usually dwarfed by the time spent
by computations using it, so efficiency of the former is not a main concern. A
practical concern on the other hand may arise from the need one often has for an
item to refer back to the previous ``parent'' item from which is was obtained.
When the items are constructed in a linked list, such a reference can be made
using a pointer or an iterator. When instead using \vect., both of these
would be invalidated by any reallocation of the vector storage while it is
growing; as a consequence, one would either need to ensure no reallocation
happens by reserving up front an amount of space guaranteed to be sufficient, or
choose to instead store an index into the vector rather than a pointer; this is
safe since the current location of the entry is looked up every time the index
is used. In some cases there may be reasons that more or less force constructing
items directly in a vector, such as items referring to each other by index in
the table (defining a graph structure of some kind); also, when the need to
identify newly generated items with any existing one of the same value is best
met by constructing a hash table, this forces (given our implementation of hash
tables) the use of a \vect. for storing items.

While all fundamental algorithms in the library were originally written using
almost exclusively vectors, we have been able to rewrite several of them making
...
Coxeter group), but for the KGB-set constructor it seems pointless to do so
because of its use of a hash table, as mentioned above. We generally found that
rewriting constructions using lists rather than vectors actually led to more
...
linked lists is suggestive of the way they will serve in the computation.
...
number of reflections acting on the lattice spanned by the same number of
``simple roots'', to complete the latter to the set of all ``positive roots'' by
closure under this action. The construction organises roots by level, the level
of a root being the sum of its coordinates expressed on the basis of simple
roots. Although both the number of levels that will be required and the total
number of positive roots produced could be computed beforehand after a detailed
study of the Cartan matrix, the original vector-based construction does not do
that; instead it reserves a sufficient number of levels based on a rough
estimate, and for the vector of positive roots does not reserve any space ahead
of time, letting repeated use of \textit{push\_\!back} determine its final size
and capacity. Each simple reflection applied to a root produces a root at a
nearby level; the level difference is at most~$3$, and if it is~$0$ then the
root is fixed by the reflection; cases where the image lands at a negative level
are few and easily singled out (it only happens when applying a reflection to
the corresponding simple root). The construction progresses from level~1
forwards, producing roots a few levels ahead, while reflections that do not
increase the level are used to set the edges of the graph representing the
reflection actions on the roots. Reflection actions that lower the level never
produce a new positive root, so once a level is reached it is complete; however,
in the levels ahead of this there may be roots missing, and since the desired
ordering of the roots is not necessarily the one in which they are first
encountered, a table index cannot yet be assigned to these roots. The structure
in which the roots are initially constructed is therefore a \vect. of, for each
level, a \std{set} of coordinate vectors. Arriving at a given level, the roots
in that set are traversed using a range-based \textbf{for}-loop, which starts by
adding the root using \textit{push\_\!back} into the vector of positive roots in
the root system; the index at which it lands will henceforth be the number of
the root in the system. Then the reflections are applied to the root; if this
increases the level, the resulting root is inserted into the root set at this
higher level, while otherwise that root is already present in the table, and
after it is located, links are added into the table between it and the current
element.

When rewriting this constructor to take advantage of linked lists, the most
obvious decision was to replace the vector of incomplete levels by a list. This
list is only used to find the level for a new root obtained by a reflection that
increases the level, and since this is always close to the current level, one
only ever needs to advance a short distance along the list. Also, once we are
done with the current level we no longer need that level on the list at all, so
we can just pop it off the front, and thus the current level is always the one
at the front of the list. (When using a vector of sets instead, we did clear the
set representing the current level when done with it, but the action does creep
...
place the information about these roots on a separate list that runs backwards,
so that the highest completed level is at the front of this list. Once the
generating procedure is complete, namely when the list of levels ahead of the
current level becomes empty, we know the number of positive roots so we can
allocate and fill the table with the information on the list of previous levels,
reversing the order.

This new organisation came with some more changes. Once a level of roots becomes
current, it is complete and the positions of all roots of the level are fixed;
we therefore change the representation to a vector when pushing the level to the
front of the list of previous levels. We also include information about the
starting index for each level on the list of previous levels, so that locating
one of its roots given it number can be done without searching, once its level
is reached; this compensates for the fact that these roots are not all stored
in a contiguous array yet.

On the list of levels \emph{ahead} of the current level, the operations to be
performed upon a level are: (1)~searching whether a newly constructed root is
already present, (2)~inserting is as a new element if not, and (3)~in either
case record the way it was obtained by a reflection from a root at a lower
level. While these operations can be performed when the level is represented as
\std{set}, as was originally done, they are actually a bit easier with a sorted
linked list representation. And although \std{set} provides binary search that
is asymptotically faster than a linear search needed with a linked list, we did
not find this argument in favour of the former persuasive, given that levels
never get very large; so we opted for initially storing levels as sorted
instances of \slls.. This choice has the added advantage of allowing the
\textit{to\_\!vector} method to perform the conversion to the \std{vector} while
using move semantics to transfer the individual roots, something that would
require manual coding if we were to convert from \std{set} to \std{vector}.

Finally, the change of representation of levels when passing from one list to
the other suggested a change in the manner the reflections links are set. When a
new root is obtained from an old root by reflection~$i$, both roots will
eventually store the number of the other root at position~$i$ in a vector of
links in the table. But this pair of mutual links cannot be set when the new
root is created, since the index of that root is not yet fixed; for this reason
setting the pair used to be done when the new root becomes current, and the
older roots is held in the list of previous levels. However, we can already
install the downward link from the new root to the current root whose, index is
known, provided the array of links is created as an additional member with the
new root, which is an easy modification with the list representation of
incomplete levels (it would be more tricky using \std{set}; indeed changing to a
\std{map} representation would be better suited to achieve this). Now when the
new root becomes current, it can locate root it links to easily within the list
of previous roots, and then set the reverse link.
...
these changes could also have been made using different container types, but
having a lightweight type adapted to small and simple situations available
greatly facilitated opting for these changes. And \slls. plays very well with
the standard library; indeed almost all handling of these lists not directly
provided by list methods is done with the aid of library functions or using
range-based loops. There is just one case where a loop over a list involves
explicit iterator manipulation, namely the one that advances a given number of
levels up along the list of incomplete levels: it must at each step ensure a
level is present before advancing, by if necessary inserting an empty level
first. In our opinion, the transformation of the code has made it much more
transparent, with singly linked lists serving as ephemeral storage that allows
postponing certain decisions until the moment the necessary size information is
available, oiling the gears of the algorithm. That the roots go through two
preliminary forms of storage before being definitively stored in the table of
the root system is a follows from and elucidates the nature of the generation
procedure.


Another
readability issue is that the natural way to set up the loop extending such a
\textit{table}, after seeding it with some initial value(s), is to write
$$
\let\&=\textbf \let\\=\textit
\&{for}~(\&{unsigned}~i=0;~ i\leq\textit{table}.\textit{size}();~\PP{i})
~\ldots
$$
%
which expresses the fact that the loop runs until the current index~$i$ catches
up with the increasing \textit{size} of~\textit{table}; the \Cpp\ semantics
ensures that the \textit{size} method is called anew after each iteration, as is
required for this to work, but it is easy to misread this to be a standard
simple loop over the indices~$i$ into a fixed~\textit{table}. In some cases
however (among which that of generating a KGB-set), we To summarise, our experience is that writing such constructions
using a singly linked list rather than a vector as storage, but sometimes is just not feasible.

We have already mentioned how, during a process of generating a collection of
values, a list embedded in a queue object can conveniently represent a
collection of values already generated but not yet inspected for generation of
further values. An alternative is to represent the whole collection as a \slls.
to which new elements are pushed (or emplaced) at the back, while the inspection
of elements for further generation is done using a separate iterator, the
procedure terminating when it manages to catch up with the back of the list.
When it is necessary to record for each values the previous value from which it
is generated, the can be done by storing the value of the latter iterator at the
moment the new value is pushed to the back.
