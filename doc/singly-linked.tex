...
\std{vector}, but they certainly do occur. As an example, some algorithms like
...
merged or reversed range, since this iterator value but would otherwise be hard
to get at afterwards. For the relative versions of~\textit{sort}, we deemed it
so unlikely that the caller would have any use for the original value of the
ending pointer of the sorted range (which has stuck to the node that came before
it, thereby becoming an iterator somewhere inside the now sorted range) that we
made this method take its second iterator argument by non constant reference,
and before returning assign the new ending iterator for the sorted range to it.
Thus it becomes easy, say, to separately sort the range up to a given iterator
and the remainder of the list, which otherwise would have been very tricky to
do correctly.

Some other methods provide common tasks that can be efficiently performed by an
appropriate call of \textit{splice} (which is what they do). These are
\textit{append} (extend at the end by another list), \textit{prepend} (the same
at the front), and \textit{insert} (at a given iterator value). They take a list
...
predicate method \textit{singleton} tests for the test for having length~$1$
(like \textit{empty} test for length~$0$), a test that we often found
useful.
...
\std{vector}$\ang{T}$ value; this facilitates using a temporary linked list to
compute the initial (or definitive) value of a vector variable or constant.
...
\frwl. than \slls.. Also there are alternative iterator types that can be
...
efficiently; however they can be recovered from the list structure alone, so
they are redundant if efficiency is ignored. And maintaining these fields does
cost some time in many methods, so in use cases where one does not invoke any
methods that depend on the presence of these fields, one can gain both time and
space by employing a structure without them. The class template \smpl. provides
such a structure.
...
\slls.\atT\textit{undress}, respectively. Since both pilfer their list
object, recycling its nodes, they leave that list in an empty state (even though
no call of \std{move} is used). In case one needs to know the length or access
the final iterators of a \smpl. just at some occasion, we provide free functions
\textit{length}, \textit{end} and \textit{cend} that when called will find these
values (by traversing the entire list); this may be preferable to converting the
list to a \slls. and then back again.
...
of the same name, returns such a pointer while emptying the list itself. In the
...
\std{vector}\atT\textit{\textbf{iterator}}, and for normal versus reverse
iterators.

To obtain weak iterators and const
weak iterators, we supplement the \textit{begin} and \textit{end} methods with
counterparts \textit{wbegin} and \textit{wend}, similarly \textit{cbegin} and
\textit{cend} methods with counterparts \textit{wcbegin} and \textit{wcend}.
These methods are defined both for \slls., but also for \smpl.; indeed \smpl.
provides \textit{wend} and \textit{wcend} methods in spite of not providing
\textit{end} or \textit{cend}, as the former just wrap a null pointer and can be
produced cheaply without first traversing the list. (Unfortunately there is no
way to instruct range-based \textbf{for} loops to use alternatives for the names
\textit{begin} and \textit{end}; if this were possible it would allow using a
such loops over \smpl. objects, using weak iterators.) Weak iterators can even
be used to traverse a list that is represented by a raw pointer to its first
node, without first requiring its conversion to an actual container, since the
weak iterator classes have a constructor from such pointers.
...
probably motivated by the idea of using \std{vector} as container), which
...
developed under the technical direction of the current author. The software
executable has two major components of rather different nature: on one hand an
extensive library of specialised mathematical constructions and algorithms
implemented in \Cpp, and on the other hand a user programmatic interface, which
provides a high level programming language that gives access to the
\Cpp\ library. This language has allowed members of the project who are not
necessarily proficient in \Cpp\ contribute new functionality on top of the
library, and indeed an extensive collection of such programs have been written
over time; it is in this area that most of the activity directed directly to
solving mathematical questions is taking place.

The \Cpp\ library is organised in several informal layers of functionality,
ranging from low level utility and support classes and functions (which is where
the implementation of linked lists is done), though code constructing internal
...
stored in tables (mostly implemented using \std{vector} containers). These
...
was often to use \std{vector} instead, or in the case of stacks and queues to
...
fact never done. As for \frwl., although it had become officially part of the
language by the time we introduced \slls., we would at that point still deemed
using it a portability liability, so it was never really considered.)

\subsection{Use of singly linked lists in queues and stacks}
...
described above, respectively. This was a drop-in replacement, with a small
added convenience that we added for placing one or more values into the data
structure at construction. As it turns out the cases where we had used these
types were almost all dynamic generation procedures, such as computing orbits
under a discrete group action. Here one starts from some element, and applies
one or more types of transformations to it to generate new elements, then
repeats these transformations for the new elements (while checking for reaching
values already encountered before, to avoid duplicates) until closure is
obtained. In this process new elements are initially generated faster than they
can be processed, so a collection of yet-to-process elements needs to be
maintained that the process can both add to and remove from, and does so until
the collection becomes empty. Mathematically it is just a set: the only
operations needed are adding an element, and testing for being non empty and if
so getting and removing an element; there is no need to insist on a selection
criterion for that element. We could therefore have used either a queue (where
the oldest element in the set is selected) or a stack (where the newest is
chosen) to implement the yet-to-process collection. In practice we invariably
chose to use a queue because a breadth first approach tends to generate elements
in a somewhat more natural order than a depth first approach, but we don't think
that using a stack would have been worse in any objective sense.

By contrast, in the mentioned cases, out choice of preferring our
\queue.$\ang{T}$ to the default \std{queue}$\ang{T}$ does involve a trade-off.
The latter involves a substantially larger up-front cost than the former when
the queue is created: while creating an empty \queue.$\ang{T}$ uses $20$ bytes
of memory none of which is dynamic, creating an empty \std{deque}$\ang{T}$
requires $\max(650,144+\textbf{sizeof}(T))$ bytes of memory, all but $80$ bytes
of which is dynamic (heap) storage; it also takes about $25$ times as long as
the latter. On the other hand the average time needed to grow the queue by a
given number of items is about $2.5$ times more when using our \queue. (probably
...
That may not be surprising in itself, as it is quite a simple data type and we
had of course done basic testing of our implementation; it is however noteworthy
that when for comparison purposes we wrote a class template derived from \frwl.
that can be used with \std{queue}, we did make some oversights, leading to
somewhat subtle errors, \emph{even though} we had already correctly handled
essentially the same problem when implementing \slls.. In both cases we need to
supplement the simply linked list with data allowing efficient appending of a
node at the end (for the \textit{push\_\!back} method), which is the final
iterator in the case of \slls., and the penultimate iterator for \frwl.; this
iterator is easily maintained when the mentioned appending happens. It must
however also be explicitly set to the initial iterator whenever calling
\textit{pop\_\!front} leaves the list empty, and when a list being
move-constructed or move-assigned happens to be empty. The final point is easy
to overlook (as we did) in the case of \frwl., if one thinks of the
\textit{before\_\!begin}() iterator a pointing to a fake node before the head of
the list; in reality it points into the \frwl.$\ang{T}$ object itself, which
clearly needs updating when the object is moved. It is in fact not explicitly
stated in the \Cpp\ standard that moving a \frwl.$\ang{T}$ object invalidates an
iterator that was obtained by a call of \textit{before\_\!begin} on the object
before the move: this can only be inferred from a charitable reading of language
that states that iterators that refer to elements of a container are not
invalidated by moving the container (instead they remain valid for the moved
container); this language excludes (usually only) past-the-end iterators, but
for \frwl. also can be taken to exclude before-the-beginning iterators.
...
cases there are no important differences with respect to using \std{vector}
instead, as we would have done if we had not had \slls. available. The most
common reason to prefer the latter is when creating a list of elements whose
length is not available beforehand; even when it can be predicted, the usage of
\std{vector} would leave us with the choice of doing that calculation and
\textit{reserve} the proper amount of entries, or not bothering because the
algorithm for growing the allocated size of a vector by exponentially growing
chunks will succeed in getting enough memory in amortised linear time anyway,
even if it might result in allocating a bit more space than necessary. By
contrast, using \slls., no decision to reserve anything is necessary, and
producing the list otherwise proceeds using repeated calls of
...
or of compactness, but even then this can simply be achieved using a final call
of \textit{to\_\!vector}. Whether or not lists perform as well as vectors would
in these cases (probably not quite) is not much of an issue, since these
constructions are usually just an initial step in the actual computation with
values produced. One practical advantage of using a linked list is that one can
store a reference to a previously constructed element using a pointer or an
iterator. When using \std{vector} neither of these is possible because any
reallocation will invalidates them; instead one can store an index into the
vector (which will lead to recomputation of the current address every time it is
used), but using (unsigned) integers whose only meaningful use is to serve as an
index to one particular vector decreases code readability.
