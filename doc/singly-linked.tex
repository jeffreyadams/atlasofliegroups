...
because of its use of a hash table. We found that such rewriting generally led
to more transparent, though not necessarily faster, code. The reasons for this
are varied. For one, trying to understand old code forces one to focus on its
...
never produce a new positive root, so once a level becomes current, it is
complete. But roots may be missing in the levels ahead of the current level, and
since the desired ordering of the roots is not necessarily the one in which they
are first encountered, a definitive root number cannot yet be assigned to these
roots. The structure into which the roots used to be initially constructed is
therefore a \vect. of, for each level, a \std{set} of coordinate vectors.
Arriving at a given level, the set is traversed using a range-based
\textbf{for}-loop. It starts by adding the root using \textit{push\_\!back} to
the vector of positive roots in the root system; the index at which it lands
defines the number of the root in the system. The reflections are then applied
one by one to the root. If this increases the level, the resulting root is
inserted into the set of roots at that level. If not, it should already by
present in the table at the current or lower level; at the current level it is
the root itself, and else it is looked up, and in both cases links in the table
are set to express the reflection relation between the roots.
...
make insertion a lot more complicated; we shall not go into details here.
...
full-fledged programming language Axis. The implementation and support for this
language represents a substantial part of the \Cpp\ code base. In the
interpreter, linked structures are much more frequently used than in the library
part, so there were more opportunities for using linked lists in this part than
...
container for this stack we use \textit{mirrored\_sl\_list}.

\subsubsection{Usage during parsing}

Linked lists are very much present in the parse tree structure that results from
reading user input. Some particularities makes their use somewhat different from
what one would expect in a normal \Cpp\ program. Our parser is produced using
the \texttt{bison} parser generator program. Thus we have no control over the
parsing function itself, which is a table-driven skeleton program written
in~\Cee, but we do provide the action functions that it calls when parts of the
user input have been recognised, to build up the corresponding parse tree. This
generated parser can actually be compiled as \Cpp~code (due to the fact that the
\Cpp\ language includes the relevant part of~\Cee), which allows the action code
to use some functionality not present in~\Cee; notably this allows it to use
\Cpp\ types in the representation of the parse tree. These types have mutually
recursive definitions that reflect the production rules of the context-free
grammar being implemented. They use pointers to refer to each other, and often
employ a tagged \textbf{union} of variants when a type represents a syntactic
category with several production rules.

Many syntactic structures, like formulas or argument lists in a function call,
have a repetitive structure. Since we do not know the number of such parts while
...
way to do this is to push the new tree to the front of that list, the result is
that the lists run backwards during their construction. The parsing
function that is called when the list gets to be incorporated in a surrounding
structure (for instance, when the closing parenthesis of an argument list is
reached) must then reverse the list before including it into that structure;
indeed this is what we systematically do in such cases.
...
formula. These actions implement an operator precedence parser, by producing as
value not a single parse tree, but a small stack of triples of a parse tree for
an operand, an operator symbol, and its precedence level (the tree is destined
to be the left operand of the operator, but its right operand is yet to be
...
After building a parse tree, if the entered input is a (as in the case of a
function definition) contains an expression, that expression is type-checked,
which process will also (upon success) produce a transformed and directly
executable version of the expression. The types used are originally just small
expressions constructed from a limited number of type constants (designating the
various primitive types that the system provides) and a small number of
constructions to combine them (forming function, row, tuple, and discriminated
union types). The possibilities have since been enlarged by the introduction of
for instance recursive and second-order types, but in spirit and internal
representation they have stayed close to the original simple repertoire. That
representation takes the form of a recursive linked structure
\textbf{\textit{type\_\!expr}} built around a
tagged union, quite similar to the representation of parse trees, but with
substantially fewer variants. The tuple and union type constructors allow for an
arbitrary number of type subexpressions, and these are represented by
$\smpl.\ang{\textbf{\textit{type\_\!expr}}}$; that type is used during the
construction of these lists, but when inserted into the \textbf{union} that
defines \textbf{\textit{type\_\!expr}}, they are converted to a raw pointer by
calling~\textit{release}, like we have seen above for lists in parse trees.
Before the introduction of our list container types, these lists were coded by an
explicit recursion, and replacing these by the container type was a very simple
change, and simplified their manipulation through the use of standard library
facilities.
